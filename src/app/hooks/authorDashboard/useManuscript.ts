...
const useManuscript = () => {
    ...
    const submitQuotation = async (formDataOne: FormDataOne, formDataTwo: FormDataTwo, formDataThree: FormDataThree) => {
        setLoading(true);

        try {
            // Upload files and collect their names
            const upldContentFileName = formDataTwo.upld_content_file && formDataOne.user_id
                ? await uploadFile(formDataTwo.upld_content_file,formDataOne.user_id,formDataOne.order_type)
                : null;
            const upldFigureFileName = formDataTwo.upld_figure_file && formDataOne.user_id
                ? await uploadFile(formDataTwo.upld_figure_file,formDataOne.user_id,formDataOne.order_type)
                : null;
            const upldTableFileName = formDataTwo.upld_table_file && formDataOne.user_id
                ? await uploadFile(formDataTwo.upld_table_file,formDataOne.user_id,formDataOne.order_type)
                : null;
    ...
    const submitQuotationJournalPublicationPackage = async (JournalPublicationFormData: PublicationFormType,formDataOne: FormDataOne, formDataTwo: FormDataTwo, formDataThree: FormDataThree) => {
        setLoading(true);
        ...
        try {
            const upldContentFileName = JournalPublicationFormData.upld_content_file && JournalPublicationFormData.user_id
                ...
                : null;
            const upldFigureFileName = JournalPublicationFormData.upld_figure_file && JournalPublicationFormData.user_id
                ? await uploadFile(JournalPublicationFormData.upld_figure_file,JournalPublicationFormData.user_id,JournalPublicationFormData.order_type)
                : null;
            const upldTableFileName = JournalPublicationFormData.upld_table_file && JournalPublicationFormData.user_id
    ...
    return {countryList,error,loading, submitQuotation, submitQuotationJournalPublicationPackage, serviceName, getServiceNameById}
}
