...
const useQuotation = () => {
    ...
    const getServiceType = async () => {
        setLoading(true)
        try {
            const response = await fetch(`https://secure.tst.manuscriptedit.com/api/get_all_service_type.php`);
            if (!response.ok) {
    ...
    const getServiceNameById = async (id: string="1") => {
        setLoading(true)
        try {
            const response = await fetch(`https://secure.tst.manuscriptedit.com/api/get_service_name.php?serv_type_id=${id}`);
            if (!response.ok) {
                throw new Error('Failed to fetch author details');
            }
    ...
    const getAllMajorSubjectType = async () => {
        setLoading(true)
        try {
            const response = await fetch(`https://secure.tst.manuscriptedit.com/api/get_all_major_subject_type.php`);
            if (!response.ok) {
                throw new Error('Failed to fetch author details');
            }
    ...
    const uploadFile = async (file: File,user_id: string,order_type: string): Promise<string> => {
        try {
            ...
            const response = await fetch("https://secure.tst.manuscriptedit.com/api/editor_order_file_upload.php", {
                method: "POST",
                body: formData,
            });
    ...
    const submitQuotation = async (formData: QuotationData) => {
        ...
        try {
            ...
            const response = await fetch(
                "https://secure.tst.manuscriptedit.com/api/submit_quotation.php",
                {
                    method: "POST",
                    headers: {
