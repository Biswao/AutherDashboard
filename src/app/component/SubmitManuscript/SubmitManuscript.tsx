...
const SubmitManuscript = () => {
  ...
  const [totalPrice, setTotalPrice] = useState<number>(0); // Local state for totalPrice
  ...
  // Update totalPrice whenever selectedServices changes
  useEffect(() => {
  ...
  const handleSubmitForm = (e: any) => {
    ...
    if (serviceTitle === "Journal Publication Packages") {
      submitQuotationJournalPublicationPackage(
        ...
        formDataTwo,
        formDataThree,
        totalPrice // Pass totalPrice to the function
      );
    } else {
      submitQuotation(formDataOne, formDataTwo, formDataThree, totalPrice); // Pass totalPrice to the function
    }
  };
  ...
  return (
    <SubmitManuscriptContext.Provider value={formDataObject}>
            ...
            servicesData.map((category) => (
              <ServiceList
                ...
                onAdd={(
                  ...
                  title: string,
                  id: string
                ) => handleAddService(serviceName, servicePrice, title, id)}
                selecetedServiceObject={selecetedServiceObject}
              />
            ))}
          {showForm &&
            (serviceTitle === "Editing Services" || serviceTitle === "Publication Support Services") ? (
            <MainStepFormComponent />
          ) : showForm && serviceTitle === "Journal Publication Packages" ? (
            <PublicationForm />
          ) : (
            ""
          )}
      ...
      >
        <button
          className={showForm ? "submit-manuscript-next-button" : ""}
        ...
        >
          {showForm && "← Back To All Services"}
        </button>
        {finalCheck && (
          <button
            className="submit-manuscript-next-button"
