...
const ServiceList: React.FC<ServiceListProps> = ({ title, services, onAdd, selecetedServiceObject }) => {
    ...
    <div style={{ backgroundColor: '#E7F4FF' }} className="text-white p-4 rounded-lg">
      ...
      {services.map((service) => (
        <div key={service.name} style={{ backgroundColor: '#D1E7F9' }} className="flex justify-between items-center bg-blue-100 p-2 mb-2 rounded-lg text-blue-900">
          ...
          <button disabled={selecetedServiceObject[service.name] ? true : false} style={{ backgroundColor: '#0D5086' }} onClick={() => onAdd(service.name, service.price)} className="text-white px-4 py-1 rounded-lg">
            Add +
          </button>
        </div>
      ))}
